...
class Rating extends Component {
  ...
  getBackground () {
    ...
    return <div className="cmpt-rating-bg">{items}</div>;
  }
    ...
    for (let i = 0, active; i < this.props.maxValue; i++) {
      ...
      items.push(
        <span key={i}
          ...
          className={classnames('cmpt-rating-handle', { active, wink: active && wink })}>
          {cloneElement(icon)}
        </span>
      );
    }

    return <div onMouseOut={this.handleLeave} className="cmpt-rating-front">{items}</div>;
  }
  ...
  getMute () {
    ...
    return (
      <div style={{ width }} className="cmpt-rating-front">
        <div className="cmpt-rating-inner">
          {items}
        </div>
      </div>
    );
