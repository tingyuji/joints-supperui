...
class Pagination extends Component {
  ...
  render () {
    ...
    if (mini) {
      items.push(
        <form key="i" onSubmit={this.setInput}>
          <input ref="input" defaultValue={this.state.index} type="text" className="cmpt-form-control" />
        </form>
      );
    ...
    let className = classnames(
      ...
      'cmpt-pagination-wrap',
      { 'cmpt-pagination-mini': mini }
    );
    return (
      <div style={this.props.style} className={className}>
        <ul className="cmpt-pagination">
          {items}
        </ul>
        {
          this.props.jumper && !mini &&
          <form onSubmit={this.setInput}>
            <div className="cmpt-input-group">
              <input ref="input" defaultValue={this.state.index} type="text" className="cmpt-form-control" />
